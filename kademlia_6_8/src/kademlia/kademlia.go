...
const (
	//TODO (if you want to test the time out function, change here in seconds)
...
func main() {
	...
	netIds := "slq0189 jjp1083 ycn2555"
	if len(netIds) == 0 {
	...
	hostname, ports, err := net.SplitHostPort(firstPeerStr)
...
func executeLine(k *libkademlia.Kademlia, line string) (response string) {
	...
	case toks[0] == "ping":
		...
		if err != nil {
			...
			if err != nil {
				response = fmt.Sprintf("ERRoo: %s", err)
			} else {
				response = "OK: " + contact.NodeID.AsString()
			}
		}
	...
	case toks[0] == "vanish":
		...
		if err != nil || numberKeys > 255{
			response = "ERR: Provided an invalid number of keys (" + toks[3] + ")"
			return
		}
		// check threshold
		threshold, err := strconv.Atoi(toks[4])
		if err != nil || threshold > 255 {
			response = "ERR: Provided an invalid threshold (" + toks[4] + ")"
			return
		} else if threshold > numberKeys{
			response = "ERR: Provided an invalid number of keys (" + toks[4] + ") larger than number of keys"
			return
		}


		k.Vanish(vdoID, []byte(toks[2]), byte(numberKeys), byte(threshold), timeoutSeconds)
		response = "Vanish complete"
		return
	case toks[0] == "unvanish":
		// perform an unvanish process
		if len(toks) != 3{
			response = "usage: vanish [Node ID] [VDO ID]"
			return
		}
		// chech nodeID
		nodeID, err := libkademlia.IDFromString(toks[1])
